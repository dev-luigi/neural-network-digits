"""
The terminal commands, and the starting point of the program: start.py (zip or git), the neural-network-digits
command (installed with pip) and python -m nn_digits all start from main().

    start.bat  (double-click)      opens the graphical interface
    python start.py                the same thing, from the terminal  (with pip:  neural-network-digits)
...
The charts of the terminal commands are saved in data/charts/ (see neural_net/storage.py for where data/ is).
"""
import argparse
import sys
...
from nn_digits.neural_net.data import download_mnist, load_photos, save_collection
from nn_digits.neural_net.network import ACTIVATIONS, NeuralNetwork
from nn_digits.neural_net.training import Trainer, robustness, test_on
from nn_digits.project import AUTHOR, LAUNCHER, NAME, VERSION
...
def draw(args):
    ...
    if not storage.MODEL_FILE.exists():
        raise SystemExit(tr("No model: train the network first  ({command})", command=f"{LAUNCHER} train"))
    from nn_digits.gui.tab_draw import open_drawing_window
    open_drawing_window()
...
def main():
    # Started with  python -m nn_digits  argparse would call the program "__main__.py" in the help
    prog = "python -m nn_digits" if sys.argv[0].endswith("__main__.py") else None
    parser = argparse.ArgumentParser(prog=prog, description=tr(
    ...
    if args.command is None:
        from nn_digits.gui.window import run  # imported only here: the terminal commands do not need it
        run()
    else:
        args.function(args)
